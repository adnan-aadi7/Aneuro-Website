...
const Systemslogstable = ({ logs = [] }) => {
  ...
  const actionIcons = {
    ...
    CREATE: <Upload className="w-4 h-4 mr-1" />,
  };
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm text-left text-gray-300">
          ...
          <tbody>
              ...
              <tr key={i} className="border-b border-gray-700 hover:bg-[#1C1F26]">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-col">
                    ...
                    <span className="text-gray-500 text-xs">{log.user?.id}</span>
                  </div>
                </td>
                <td className="px-4 py-3">
                  <span
                    className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded ${actionStyles[log.action] || 'bg-gray-100 text-gray-700'}`}
                  >
                ...
                <td className="px-4 py-3">
                  <span
                    ...
                    className={`px-3 py-1 text-xs font-medium cursor-pointer ${severityStyles[log.severity] || 'bg-gray-100 text-gray-800'}`}
                  >
                    {log.severity}
                  </span>
            ...
            {logs.length === 0 && (
              <tr>
                <td colSpan="6" className="text-center text-gray-500 px-4 py-6">No logs found</td>
              </tr>
            )}
          </tbody>
        </table>
  ...
  <Modal onClose={() => setShowModal(false)} title={`${selectedLog.action} Action Details`}>
    <div className="space-y-4 text-sm text-white p-4 md:w-[800px]">
      ...
      <div className="p-3 border border-[#212C3B] bg-[#23232F]">
        ...
        <div className="text-[#66BDCC]">{selectedLog.affectedAsset}</div>
      </div>
      <div className="p-3 border border-[#212C3B] bg-[#23232F]">
      ...
      <div className="p-3 border border-[#212C3B] bg-[#23232F]">
        ...
        <div className="text-[#66BDCC]">User: {selectedLog.user?.email}</div>
        <div className="text-[#66BDCC]">User ID: {selectedLog.user?.id}</div>
        <div className="text-[#66BDCC]">IP Address: {selectedLog.user?.ipAddress}</div>
        <div className="text-[#66BDCC]">Timestamp: {formatDate(selectedLog.timestamp)}</div>
      </div>
      <div className="p-3 border border-[#212C3B] bg-[#23232F]">
