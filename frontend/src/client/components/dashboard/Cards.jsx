...
const Cards = () => {
  const [weeklyTools, setWeeklyTools] = useState(0);

  // Fetch weekly tools from API
  useEffect(() => {
    const fetchWeeklyTools = async () => {
      try {
        const res = await axiosInstance.get("/recently-added-tools/weekly-tools" );
        if (res.data?.success) {
          setWeeklyTools(res.data.data?.total || 0);
        }
      } catch (err) {
  ...
  const cards = [
    ...
    },
    {
      ...
      subtitle: "New tools have been uploaded by Aneuro Admin",
      value: weeklyTools, // ✅ from API
      valueLabel: "tools",
      week: "↑ Week 1",
    },
    {
