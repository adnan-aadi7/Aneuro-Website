...
export default function ReflectorPrompt({ groupedPrompts = {}, categories = [] }) {
  ...
  const promptsForReflector = useMemo(() => {
    ...
    return selectedCategory ? reflectorPrompts.filter((p) => p.category === selectedCategory) : reflectorPrompts;
  ...
  const availableCategories = useMemo(() => {
    ...
    const set = new Set();
    (groupedPrompts.Reflector || []).forEach((p) => p.category && set.add(p.category));
    return Array.from(set);
  }, [categories, groupedPrompts.Reflector]);

  const prompt1 = promptsForReflector[0] || null;
  const prompt2 = promptsForReflector[1] || null;
      ...
      <div className="bg-[#303041] text-white mt-10">
        <div className="p-2 lg:p-8">
          ...
          <div className="bg-[#23232F] p-8 text-center">
            <p className="text-gray-400 text-lg">No prompts found for Reflector brain type</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="p-2 lg:p-8">
          ...
          <div className="bg-[#23232F] p-6 mb-4 relative" onMouseEnter={() => setShowEmailTooltip(true)} onMouseLeave={() => setShowEmailTooltip(false)} onMouseMove={(e) => { const rect = e.currentTarget.getBoundingClientRect(); setEmailTooltipPos({ x: e.clientX - rect.left, y: e.clientY - rect.top }); }}>
            {showEmailTooltip && (
              <div className="pointer-events-none bg-black text-white text-[10px] px-2 py-1 rounded shadow-lg z-20 whitespace-nowrap" style={{ position: "absolute", left: emailTooltipPos.x + 10, top: emailTooltipPos.y + 10, minWidth: "max-content", maxWidth: 180 }}>
                This section generates a strategic social media caption email prompt, including subject, copy button, and example message.
              </div>
            )}
            <div className="flex justify-between items-start mb-4">
              <div className="flex-1">
                <h3 className="text-base font-medium mb-2">Strategic Social Media Caption Generator</h3>
                <p className="text-cyan-400 text-sm mb-4">Subject: Plan for success with this strategy!</p>
              </div>
        ...
        {hasPayload(prompt2) && (
          <div className="bg-[#23232F] p-6">
            <div className="flex justify-between items-start mb-4">
              <div className="flex-1">
                <h3 className="text-base font-medium mb-2">Strategic Problem-Solution Framework</h3>
                <p className="text-cyan-400 text-sm mb-4">Subject: Solving strategic challenges with proven methods...</p>
              </div>
